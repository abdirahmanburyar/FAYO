...
export class MessageQueueService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.kafka = new Kafka({
      clientId: 'shared-service',
      brokers: [process.env.KAFKA_BROKER || 'localhost:9092'],
    });

    this.producer = this.kafka.producer();
    this.consumer = this.kafka.consumer({ groupId: 'shared-service-group' });

    await this.producer.connect();
    await this.consumer.connect();
    
    // Subscribe to request topics
    await this.consumer.subscribe({ topic: 'shared-service-requests', fromBeginning: false });
  ...
  private async handleMessage(topic: string, message: any): Promise<void> {
    try {
      ...
      if (topic === 'shared-service-requests') {
  ...
  private async sendResponse(correlationId: string, response: MessageQueueResponse): Promise<void> {
    try {
      await this.producer.send({
        topic: 'shared-service-responses',
        messages: [{
          key: correlationId,
