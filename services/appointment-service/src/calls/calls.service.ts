...
export class CallsService {
  ...
  async createCall(createCallDto: CreateCallDto) {
    try {
      this.logger.log(
        `📞 [CALL] Creating video call for appointment: ${createCallDto.appointmentId}`,
      );
      ...
      this.logger.log(
        `✅ [CALL] Video call session created: ${channelName} for appointment: ${appointment.id}`,
      );
      ...
      this.logger.error(`❌ [CALL] Error creating call:`, error);
      if (
  ...
  async getParticipantCredentials(appointmentId: string, userId: string) {
    try {
      this.logger.log(
        `📞 [CALL] Getting participant credentials for appointment: ${appointmentId}, user: ${userId}`,
      );
      ...
      this.logger.log(
        `✅ [CALL] Participant credentials generated for appointment: ${appointmentId}`,
      );
      ...
      this.logger.error(`❌ [CALL] Error getting participant credentials:`, error);
      if (
        error instanceof NotFoundException ||
        error instanceof BadRequestException
      ) {
  ...
  async endCall(appointmentId: string, userId: string) {
    try {
      this.logger.log(
        `📞 [CALL] Ending call for appointment: ${appointmentId}, user: ${userId}`,
      );
      ...
      this.logger.log(
        `✅ [CALL] Call ended for appointment: ${appointmentId}`,
      );
  ...
  async acceptCall(appointmentId: string, patientId: string, channelName: string) {
    try {
      this.logger.log(
        `📞 [CALL] Patient ${patientId} accepted call for appointment: ${appointmentId}`,
      );
      ...
      this.logger.log(
        `✅ [CALL] Call accepted event broadcasted for appointment: ${appointmentId}`,
      );
      ...
      this.logger.error(`❌ [CALL] Error accepting call:`, error);
      throw error;
    }
  }
}
