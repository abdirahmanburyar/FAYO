...
Uses Docker to connect to PostgreSQL container.
"""
...
# Database configuration - update these based on your setup
DB_CONFIG = {
...
def create_backup(db_name: str) -> Optional[str]:
    ...
    # First, create backup inside container using docker exec
    cmd = [
        "docker", "exec",
    ...
    try:
        logger.info(f"Creating backup for database: {db_name} (using Docker container: {DOCKER_CONTAINER})")
        ...
        logger.debug(f"Copying backup from container to host...")
        result = subprocess.run(
            copy_cmd,
            capture_output=True,
            text=True,
        ...
        logger.error("docker command not found. Please install Docker and ensure it's in your PATH.")
        return None
    except Exception as e:
...
def backup_database(db_name: str, metadata: dict):
    ...
    # Create new backup
    new_backup_file = create_backup(db_name)
...
def main():
    ...
    try:
        ...
        if DOCKER_CONTAINER not in result.stdout:
            logger.warning(f"Container '{DOCKER_CONTAINER}' may not be running. Attempting backup anyway...")
    except Exception as e:
        logger.warning(f"Could not verify Docker container status: {e}")
