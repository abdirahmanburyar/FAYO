...
// Validation schema for hospital login
...
type HospitalLoginForm = z.infer<typeof hospitalLoginSchema>;
...
export default function HospitalLoginPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<HospitalLoginForm>({
    resolver: zodResolver(hospitalLoginSchema),
  });

  const onSubmit = async (data: HospitalLoginForm) => {
    setIsLoading(true);
    setError('');

    try {
      // Call hospital login endpoint
      ...
      if (response.ok) {
        // Store hospital token and data
        localStorage.setItem('hospitalToken', result.accessToken);
        localStorage.setItem('hospitalRefreshToken', result.refreshToken);
        localStorage.setItem('hospitalUser', JSON.stringify(result.user));
        ...
        >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  type="text"
                  id="username"
                  placeholder="Enter hospital username (numbers only)"
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-gray-500">
              FAYO Healthcare Hospital Panel
            </p>
          </div>
        </motion.div>
      </motion.div>
