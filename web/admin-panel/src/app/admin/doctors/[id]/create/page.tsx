...
export default function CreateDoctorPage() {
      ...
      if (isEditMode && doctorId) {
        try {
          ...
          setFormData({
            ...
            gender: (userData?.gender || 'MALE') as 'MALE' | 'FEMALE' | 'OTHER',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && doctorId) {
        // Edit mode - get existing doctor's user
        const doctorData = await doctorApi.getDoctorById(doctorId);
      ...
      if (isEditMode && doctorId) {
        ...
        const updateData: UpdateDoctorDto = {
          // Always include specialtyIds if array exists (even if empty to clear specialties)
          ...(Array.isArray(formData.specialtyIds) && {
          ...
          // Handle self-employed consultation fee: set to null if not self-employed, or set value if self-employed
          ...(formData.isSelfEmployed !== undefined && {
            selfEmployedConsultationFee: (formData.isSelfEmployed && formData.consultationFee > 0 
              ? Math.round(formData.consultationFee * 100) 
              : null) as number | null
          }),
