...
export default function SpecialtiesPage() {
  ...
  const fetchSpecialties = async () => {
    try {
      setLoading(true);
      setError(null);
      const specialtiesData = await specialtiesApi.getSpecialties(undefined, true); // Include inactive
      setSpecialties(specialtiesData);
    } catch (error) {
      console.error('Error fetching specialties:', error);
      setError(error instanceof Error ? error.message : 'Failed to fetch specialties');
    } finally {
      setLoading(false);
    }
  };
