...
import { sharedApi, Service, CreateServiceDto } from '@/services/sharedApi';
...
export default function ServiceManager() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const servicesData = await sharedApi.getServices();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingService) {
        await sharedApi.updateService(editingService.id, formData);
      } else {
        await sharedApi.createService(formData);
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await sharedApi.deleteService(id);
      await loadData();
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage medical services and their categories</p>
        </div>
        <button
